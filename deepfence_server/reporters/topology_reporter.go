...
func (nc *neo4jTopologyReporter) GetJSONGraph(ctx context.Context, filters TopologyFilters) (string, error) {
	res, err := nc.GetGraph(ctx, filters.cloud_filter, filters.region_filter, filters.host_filter)
	if err != nil {
		return "", err
	}
...
func NewNeo4jCollector(ctx context.Context) (*neo4jTopologyReporter, error) {
