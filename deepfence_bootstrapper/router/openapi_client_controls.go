...
func (ct *OpenapiClient) StartControlsWatching(nodeID string, isClusterAgent bool, version, nodeType string) error {
	if isClusterAgent {
...
func GetScannersWorkloads() int32 {
	...
	log.Info().Msgf("workloads = vuln: %d, secret: %d, malware: %d, cloud: %d", vuln, secret, malware, cloud)
